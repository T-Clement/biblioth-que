document.addEventListener("DOMContentLoaded", function () {
  ...
  const activationAPI = document.querySelector("#api-activation:checked");
  ...
  addTitleBookInput.addEventListener("keyup", function (e) {
	...
	// not display execute code if api search option is activated
	...
	timer = setTimeout(function () {
		...
		getTitlesFromApi(e.target.value).then((response) => {
			...
			li.addEventListener("click", function (e) {
				...
				addAuthorBookInput.value = element["author_name"][0]; // author_name is an array
				titleSuggestions.innerHTML = "";
...
function generateUniqBookId(titleInput, authorInput) {
  ...
  return new String((authorCode + titleCode + randomFourDigitsNumber).toUpperCase());
}
